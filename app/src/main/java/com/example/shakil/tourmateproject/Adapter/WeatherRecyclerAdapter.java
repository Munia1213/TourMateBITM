package com.example.shakil.tourmateproject.Adapter;
...
import com.example.shakil.tourmateproject.Model.WeatherViewHolder;
...
public class WeatherRecyclerAdapter extends RecyclerView.Adapter<WeatherViewHolder> {
    ...
    public void onBindViewHolder(WeatherViewHolder customViewHolder, int i) {
        ...
        if (sp.getString("speedUnit", "m/s").equals("bft")) {
            customViewHolder.itemWind.setText(context.getString(R.string.wind) + ": " +
                    UnitConvertor.getBeaufortName((int) wind) + " " + WeatherInfo.getWindDirectionString(sp, context, weatherItem));
        } else {
            customViewHolder.itemWind.setText(context.getString(R.string.wind) + ": " + new DecimalFormat("#.0").format(wind) + " " +
                    WeatherInfo.localize(sp, context, "speedUnit", "m/s")
                    + " " + WeatherInfo.getWindDirectionString(sp, context, weatherItem));
        }
        customViewHolder.itemPressure.setText(context.getString(R.string.pressure) + ": " + new DecimalFormat("#.0").format(pressure) + " " +
                WeatherInfo.localize(sp, context, "pressureUnit", "hPa"));
